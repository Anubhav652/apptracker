from sheets import Sheets, JobStatus
from joblist_retriever import ListingRetriever, JobListing
import threading
...
class Main:
    def __init__(self):
        self.sheets = Sheets()
        self.retriever = ListingRetriever()

        self.retriever_list : dict[str, dict[str, list[JobListing]]] = {}
        # self.retriever_list[ COMPANY NAME ][ JOB TITLE ] = list of job listings corresponding to this
        ...
        for company_name, _data in self.retriever_list.items():
            for job_title, job_listings in _data.items():
                for lsting in job_listings:
                    ...
                    if lsting.url not in Settings.JOB_LISTINGS_ACTUAL_LINKS.values():
